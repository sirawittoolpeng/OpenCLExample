...
int main()
{
    // ProfilerStart("inv.prof"); // for CPU profilling by google perf tools
    if (!ocl::haveOpenCL())
      {
    ...
    cv::String errmsg;
    cv::String buildopt = "-DDBG_VERBOSE ";
    ocl::Program program = context.getProg(programSource, buildopt, errmsg); // Compile OpenCL code (OpenCL/kernel_inv.cl)
    ...
    cout << "Input image size: " << mat_src.size() << endl
        << flush;
    ...
    bool success = kernel.run(2, globalThreads, localThreads, false); // Run the OpenCL kernel
